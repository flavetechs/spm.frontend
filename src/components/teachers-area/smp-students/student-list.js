...
const StudentList = () => {
  ...
  const DeActivateStudent = async (staffId, activate) => {
    try {
      debugger
      const msg = activate == false ? 'Successfully activated student' : 'Successfully de-activated student';
      const confirm = await SweatAlert.confirmAction(activate == false ? `You want to activate student` : `You want to deactivate student`)
      if (confirm) {
        await axiosInstance.post("/smp/server/student/api/v1/de-activate/student", { id: staffId });
        getAllStudents(1)(dispatch);
        debugger
        if (activate == true)
          SweatAlert.showSuccess(msg);
        else
          SweatAlert.showSuccess(msg);
      }
                                ...
                                {" "}
                                  ...
                                  >
                                    <Link
                                      ...
                                      data-id={student.studentAccountId}
                                      onClick={() => {
                                        DeActivateStudent(student.studentAccountId, student.status == "active")
                                      }}
                                    >
                                      <span className="btn-inner">
                                        <svg
