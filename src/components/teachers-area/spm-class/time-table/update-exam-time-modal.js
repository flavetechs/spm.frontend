...
import { updateExamTimetableTime } from "../../../../store/actions/timetable-actions";

export function UpdateExamTimeModal({ selectedClassId, currentPeriod, timetableTimeId }) {
    ...
    React.useEffect(() => {
        ...
        newTime.end = endTime
        newTime.examTimeTableTimeId = timetableTimeId
    }, [currentPeriod]);
    ...
    const handeSubmit = () => {
        if (!newTime.start.trim() || !newTime.end.trim()) {
            ...
            updateExamTimetableTime(newTime, selectedClassId)(dispatch);
    ...
    React.useEffect(() => {
        if (submitSuccessful) {
            setNewTime({ start: startTime, end: endTime, examTimeTableTimeId: timetableTimeId });
        }
    }, [submitSuccessful]);
