...
const SubjectSetupList = () => {
  ...
  const checkSingleItem = (isChecked, subjectId, subjects) => {
    subjects.forEach(item => {
      if (item.lookupId === subjectId) {
        item.isActive = isChecked
      }
    });
    if (isChecked) {
      dispatch(pushId(subjectId));
    } else {
      dispatch(removeId(subjectId));
    }
  }
                          ...
                          <td>
                            <div className="flex align-items-center list-user-action">
                              ...
                              <Link
                                ...
                                to={`${classLocations.roleEdit}?roleId=${item.lookupId}`}
                              >
                                <span className="btn-inner">
                                  <svg
                                    width="20"
