...
import { getSingleWardsNote } from "../../../store/actions/parent-actions";
import { closeFullscreen, openFullscreen } from "../../../utils/export-csv";

const WardsNoteDetails = () => {
  const state = useSelector((state) => state);
  const { selectedWardsNote } = state.parent;
  ...
  const studentNoteIdQuery = queryParams.get("studentNoteId");

  useEffect(() => {
    getSingleWardsNote(studentNoteIdQuery)(dispatch);
      ...
      <div className="col-md-12 mx-auto">
        <Row>
                ...
                <Card.Body>
                  <div className="d-flex justify-content-between mt-3 flex-wrap">
                    ...
                    <div>
                      ...
                      <span className="text-end text-primary">
                        {" "}
                        {selectedWardsNote?.approvalStatusName}
                      </span>
                    </div>
                  </div>
                  <div className="mt-2 d-md-flex justify-content-end">{selectedWardsNote?.subjectName}</div>
                  <div className="mt-2 d-md-flex justify-content-end">Submitted on: {selectedWardsNote?.dateSubmitted}</div>
                  <div className="d-md-flex justify-content-start mt-2">
                    ...
                    <div className="ms-2 mt-2 fw-bold">
                      <span>{selectedWardsNote?.noteTitle}</span>
                      <br />
                    </div>
                  </div>
                  <div
                    dangerouslySetInnerHTML={{
                      __html: selectedWardsNote?.noteContent,
                    }}
                  ></div>
                  <hr />
                </Card.Body>
          ...
          <Col sm="5">
            <Card>
              <Card.Body>
                <h4 className="mb-4">Student's Name</h4>
                <div className="d-flex gap-3">
                    <h6 className="mb-3 text-primary">
                      {selectedWardsNote?.studentName}
                    </h6>
                </div>
              </Card.Body>
            </Card>
...
export default WardsNoteDetails;
