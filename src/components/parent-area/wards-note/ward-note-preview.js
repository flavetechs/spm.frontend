...
const WardsNotePreview = ({ selectedSubjectId }) => {
    ...
    const { myWardsNotes, filterProps } = state.parent;
    // ACCESSING STATE FROM REDUX STORE

    const queryParams = new URLSearchParams(locations.search);
    const classIdQuery = queryParams.get("classId");
    const wardNameQuery = queryParams.get("ward");
    const studentContactIdQuery = queryParams.get("studentContactId");

    React.useEffect(() => {
        if (classIdQuery && selectedSubjectId && studentContactIdQuery) {
            getAllMyWardsNotes(1, classIdQuery, selectedSubjectId, studentContactIdQuery)(dispatch);
        }
    }, [dispatch, classIdQuery, selectedSubjectId, studentContactIdQuery]);
    ...
    return (
        <>
            ...
            <Card.Footer>
                <PaginationFilter3 filterProps={filterProps} action={getAllMyWardsNotes} dispatch={dispatch} param1={classIdQuery} param2={selectedSubjectId} param3={studentContactIdQuery} />
            </Card.Footer>
        </>
    );
};

export default WardsNotePreview;