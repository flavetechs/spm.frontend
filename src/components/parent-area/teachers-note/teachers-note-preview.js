...
const TeachersNotePreview = ({ selectedSubjectId }) => {
  ...
  const { myWardsClassNotes, filterProps } = state.parent;
  // ACCESSING STATE FROM REDUX STORE
    ...
    <>
      ...
      <Card.Body>
        ...
        {myWardsClassNotes.length < 1 ?
          ...
          :
          <Row className="">
            {myWardsClassNotes?.map((item, idx) => (
...
export default TeachersNotePreview;