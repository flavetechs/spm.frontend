...
import { getSingleWardsClassNote } from "../../../store/actions/parent-actions";
import { closeFullscreen, openFullscreen } from "../../../utils/export-csv";

const TeachersNoteDetails = () => {
  const state = useSelector((state) => state);
  const { selectedWardsClassNote } = state.parent;
  ...
  const teacherClassNoteIdQuery = queryParams.get("teacherClassNoteId");

  useEffect(() => {
    getSingleWardsClassNote(teacherClassNoteIdQuery)(dispatch);
                ...
                <Card.Body>
                  <div className="d-md-flex justify-content-between mt-3 flex-wrap">
                    ...
                    <div>
                      ...
                      <span className="text-end text-primary">
                        {" "}
                        {selectedWardsClassNote?.approvalStatusName}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="d-md-flex justify-content-start mt-2">
                    ...
                    <div className="ms-2 mt-2 fw-bold">
                      <span>{selectedWardsClassNote?.noteTitle}</span>
                      <br />
                    </div>
                  </div>
                  <div
                    dangerouslySetInnerHTML={{
                      __html: selectedWardsClassNote?.noteContent,
                    }}
                  ></div>
                  <hr />
                </Card.Body>
              ...
              <Card.Body>
                ...
                <div className="d-flex align-items-center gap-3">
                  <img
                    ...
                    src={selectedWardsClassNote?.noteAuthordetail?.photo}
                    alt="user-img"
                  />
                  <div>
                    <h6 className="mb-3 text-primary">
                      {selectedWardsClassNote?.noteAuthordetail?.fullName}
                    </h6>
                    <p className="mt-3">
                      {selectedWardsClassNote?.noteAuthordetail?.shortBio}
                    </p>
                  </div>
                </div>
              </Card.Body>
...
export default TeachersNoteDetails;
