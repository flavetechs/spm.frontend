...
const ParentExamTimeTableActivities = () => {
  ...
  const { myWardsExamTimetable } = state.parent;
  // ACCESSING STATE FROM REDUX STORE
              ...
              <Card.Header className="d-flex justify-content-between">
                <div className="header-title">
                  <h4 className="card-title">
                    <span>{`${myWardsExamTimetable?.className} Exam Timetable`}</span>
                  </h4>
                </div>
                <div>
                  <Button className="text-center btn-primary btn-icon mt-lg-0 mt-md-0 mt-3 ms-2"
                    onClick={() => {
                      history.push(
                        `${parentTimeTableLocations.printTimeTable}?classLkId=${classLkIdQuery}&timetableType=examTimeTable`
                      );
                    }}
                  >
                ...
                <div className="table-responsive">
                  <table className="table striped='column' table-bordered border-3 table-striped">
                    <thead className="border border-3">
                      <tr>
                        <th></th>
                        {myWardsExamTimetable?.timetable?.days?.map((items, index) => (
                    ...
                    <tbody className="border border-3">
                      {myWardsExamTimetable?.timetable?.times?.map((item, index) => (
                        <tr key={index}>
...
export default ParentExamTimeTableActivities;
